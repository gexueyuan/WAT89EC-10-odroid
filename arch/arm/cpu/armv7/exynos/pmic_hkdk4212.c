...
void pmic_init(void)
{
	...
	rwdata = 0xD4;		// 1.8V Enable
	pmic_write(0x53, &rwdata, 1);	// EMMC

	rwdata = 0xAC;		// 3.0V Enable
	pmic_write(0x56, &rwdata, 1);	// TOUCH
	pmic_write(0x57, &rwdata, 1);	// TOUCH LED
	pmic_write(0x58, &rwdata, 1);	// VDDQ_LCD
	pmic_write(0x59, &rwdata, 1);	// MOTOR
}
//[*]----------------------------------------------------------------------------------------------[*]
//[*]----------------------------------------------------------------------------------------------[*]
